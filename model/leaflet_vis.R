...
result = apply(df_tp2, 1, function(row_data) {
nrow(
    df_hda_trt[
      df_hda_trt$Latitude_SW <= as.numeric(row_data['LATITUDE'])
      & as.numeric(row_data['LATITUDE']) <= df_hda_trt$Latitude_NE
      & df_hda_trt$Longitude_SW <= as.numeric(row_data['LONGITUDE'])
      & as.numeric(row_data['LONGITUDE']) <= df_hda_trt$Longitude_NE
      ,
    ]
  )
})
