...
p %>% mutate(clear = visibility == "Clear") %>%
  group_by(clear, year) %>%
  summarize(prop_fat = sum(acc_class == "Fatal")/n()) %>%
...
```{r echo = F}
accidents <- read.csv("https://raw.githubusercontent.com/sergiosonline/data_sci_geo/master/data/accidents.csv", check.names = F)
...
The fixed effects of this model contains are *visibility*, *types of road*, *traffic control* and *Precipitation*. Those covariantes used in the model are unrelated to the personel involved in the accidents, so factors such as condition of the drivers are not included.
...
accidents_time <- accidents4 %>% group_by(hood_id, year,month) %>% summarise(value_perd=n())
...
```{r echo=F}
accidents_ts_y = accidents4 %>% group_by( year,month) %>% summarise(value_perd=n())
