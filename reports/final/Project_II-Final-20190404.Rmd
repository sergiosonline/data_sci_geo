...
We define the city as per the guidelines set by the [City of Toronto](https://www.toronto.ca/city-government/data-research-maps/neighbourhoods-communities/neighbourhood-profiles/). Below are the neighborhood limits and the official 2016 population census estimates:
...
```{r echo=F, fig.pos='H', fig.align='center', out.width='75%', fig.cap="\\label{fig:figs}Population by neighborhood of the City of Toronto in the census year 2016", warning=F, message=F}
...
For our analysis we employed data from the [Toronto Police Service](http://data.torontopolice.on.ca), the [City of Toronto](https://www.toronto.ca/city-government/data-research-maps), and [Environment Canada](http://climate.weather.gc.ca/historical_data/search_historic_data_e.html). Each of these datasets contain different levels of granularity and information, and were therefore combined to obtain the following variables of interest outlined in **Appendix: Dataset Variables and Definitions**.
...
Traffic incident information provided by Toronto Police served as a base for the data used for this analysis. There are 3,902 unique accidents in this dataset. Each of the 11,360 entries represent a party involved in a traffic collision event. Other features such as the location of the collision (intersection, neighborhood, ward), road condition (visibility and road precipitation), driver action (e.g. speeding, involved alcohol), and types of vehicles (e.g. automobile, pedestrian, cyclist) involved were also used.

Population counts for 2011 and 2016 are available through the national census for each neighborhood. The populations for dates not provided by the census were extrapolated using a linear growth model. 
...
```{r echo=F, fig.pos='H', fig.align='center', out.width='50%'}
## Visualizing neighborhoods of Toronto for reference
...
Using this application for our exploratory analysis allowed us to uncover some interesting trends quite quickly. Another advantage was that it provided a way to check whether a feature was reasonable to include into our models. Moreover, because it is relatively simple to add inputs and functionality, we were able to customize it to meet our needs dynamically.

Firstly, the number of accidents did not appear to increase with year even though the population of Toronto grew by a considerable amount from 2007 to 2017. While it's great news for Torontonians, this could be a result of many factors. While it would be favorable to assume that the number of accidents decreased due to the efforts of the City to improve road safety, it is equally likely to have been because the Toronto Police were less rigorous with their data keeping, or that people involved in accidents were less likely to report it and get the police involved. In addition, while the total number of accidents have been going down, the number of fatal accidents have remained stable, causing the proportion of fatal accidents to actually increase by year.
...
Surprisingly, if it had precipitated the day of the accident, there were periods of time where it was less likely to find a fatal accident. This may be because bad weather deters people from going outside and driving. Note that due to data limitations, the type of precipitation was not distinguished.
...
By summing up counts from 2007 to 2017, Don Valley Village appeared to be the deadliest intersection followed by South Parkdale, then West Humber-Clairville. Thankfully, the fatalities appeared to be quite low compared to the total number of collisions reported by the Toronto Police.
...
The rankings for deadliest neighborhoods change when looking at accidents that involved pedestrians or cyclists. Keeping in mind the counts are over all 11 years, the total number of fatal accidents involving pedestrians and cyclists are extremely low and therefore a ranking of deadliest neigborhoods for these accidents are not explored further.
...
Mixed effects logistic regression is used to model binary outcome variables in which the odds of the outcomes are modelled as a linear combination of the predictor variables when data are clustered (random effects). This mixed effect model is used to describe the binomial probability of an auto accident resulting in a fatality. It takes into account not just unobserved differences between neighborhoods, but also the evolution of these odds through time with the inclusion of semi-parametric terms.
\begin{equation}
...
The fixed effects of this model are *visibility*, *types of road*, *traffic control* and *Precipitation*. The covariates used in the model are unrelated to the personel involved in the accidents, so factors such as condition of the drivers are not included.
...
accident_ts_gam = gam(value ~ monthf + offset(log_pop) + s(day_num) + s(hood_id,bs="re"), data=accidents_ts2, family='poisson')
...
A spatial model Log Gaussian-Cox Process (LGCP) was considered to fit the accident counts in 2017 with intercept only.
...
- The odds of having fatality are higher when driving on the highway.
- Having traffic signage and traffic light leads to a lower odds, compared to no control.
- Accidents without pedestrian (vehicle to vehicle) involved have lower odds of having a fatality
- The odds of fatality are slightly lower when there is more precipitation. -2% odds of fatality with 1mm of precipitation increasing. This may be due to drivers slowing down when they have difficulty seeing ahead or when knowing road is slippery.

The time trend graph below shows the odds of fatality rising until mid-2016 and then decreasing. This is consistent with press reports deeming 2015-2017 as bad years for Toronto in terms of fatality. The decrease post-2017 could be attributed to the [Vision Zero](https://www.toronto.ca/services-payments/streets-parking-transportation/road-safety/vision-zero/vision-zero-plan-overview/) municipal plan to address road fatalities.
...
knitr::kable(summary(accident_ts_gam)$p.table[,1:2],digits=3, escape=F, format="latex", booktab=T,linesep = "", caption="Estimated coefficients and their standard errors. Here the baseline month is January.")  %>% 
  kable_styling(latex_options = "hold_position") 

```

```{r echo = F,fig.pos='H', fig.align='center', out.width='65%', message=F}
# check 1 hood
# hood_id=77
...
From the table above we notice summer has a higher number of accidents in general. This may be because there are more drivers on the road and perhaps more cars racing illegally. It may also be due to the caution drivers put in their driving in the winter especially when roads are icy or snowy. A rate ratio plot below show this same pattern.

```{r echo = F,fig.pos='H', fig.align='center', out.width='65%', message=F,fig.cap="\\label{fig:figs}Estimated seasonality of accidents"}
#plot rr by month
accident_ts_gam_pred_rr = exp(summary(accident_ts_gam)$p.table[2:12,1:2] %*% Pmisc::ciMat())
...
Below are plots of the number of accidents that occurred per month in Toronto and the predictions of accidents in neighbourhood 5 (Elms-Old Rexdale) and 122 (Birchcliffe-Cliffside) from GAM. In gereral they capture a trend that the monthly number of accidents are descreasing by year.
...
```{r echo = F, fig.pos='H', fig.align='center', out.width=c('45%','45%'), message=F,fig.subcap=c('Elms-Old Rexdale', 'Birchcliffe-Cliffside'),fig.cap="\\label{fig:figs}Estimated accident counts per neighborhood through the years"}
plot_predict_hoodid(5)

plot_predict_hoodid(122)
```

## Log Gaussian-Cox Process Spatial Model

The plots below correspond to the observed and expected accident counts ($\lambda$) in Toronto in the year 2017 as estimated by our model. We could see that many accidents are expected in downtown area and along Yonge street. More traffic control may be required, perhaps in the shape of better signage or human control. 

```{r echo=F, eval=F,fig.pos='H',fig.align='center',out.width=c('45%','45%'),fig.subcap=c('Observed', 'Estimated'),fig.cap="\\label{fig:figs}Observed and estimated accident counts in Toronto in the year 2017"}
